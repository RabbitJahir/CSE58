...
import { oopCodes } from "./OopCodes"; // import from the same folder
    ...
    <div className="min-h-screen flex flex-col" >
      ...
      {oopCodes.map((snippet, index) => (
        <div key={snippet.id} className="bg-black/20 p-4 rounded-lg text-white relative">
          ...
          >
            <span>{index + 1}. {snippet.title}</span>
          ...
          {open === snippet.id && (
            <div className="mt-3 relative">
              <pre className="overflow-x-auto bg-black/50 p-3 rounded text-sm font-mono">{snippet.code}</pre>
              <button
